...
const requestHandler=(req,res)=>{
  ...
  if(req.url==="/version"){res.writeHead(200);res.end(JSON.stringify({service:APP_NAME,version}));return;}
  res.writeHead(404);res.end(JSON.stringify({error:"not found"}));
};

if(require.main===module){
  http.createServer(requestHandler).listen(PORT,()=>console.log(`${APP_NAME} listening on ${PORT}`));
}
